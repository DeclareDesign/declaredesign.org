...
::: {.g-col-6 .hero-left .align-items-center}
...
::: {.g-col-6 .hero-right .text-center}
![](https://book.declaredesign.org/figures/cover.jpg){width=400}
:::
...
::: {.g-col-lg-6 .g-col-12}
````markdown
redesign(two_arm_trial, sample_size = c(250, 500, 750, 1000, 1250, 1500))
````
...
#| fig.height: 3
#| echo: false
designs_ss <- redesign(two_arm_trial, sample_size = c(250, 500, 750, 1000, 1250, 1500))
diagnosis_ss <- diagnose_design(designs_ss, sims = 1000)
diagnosands <- diagnosis_ss |> tidy() |> filter(diagnosand == "power")
ggplot(diagnosands, aes(sample_size, estimate)) + 
  # geom_ribbon(aes(ymin = conf.low, ymax = conf.high), fill = "lightblue", col = NA) + 
  geom_smooth() +
  geom_hline(yintercept = 0.8, color = "red", size = 1, lty = "dashed") + 
  geom_curve(
    data = data.frame(x1 = 600, x2 = 760, y1 = 0.9, y2 = 0.82),
    aes(x = x1, y = y1, xend = x2, yend = y2),
    arrow = arrow(length = unit(0.01, "npc"))
  ) + 
  annotate("text", x = 450, y = 0.94, label = "Minimum sample size with\n at least 80% power", ) + 
  # annotate("text", x = diagnosands$mean_estimand + 0.025, y = 100, hjust = 0, label = "Estimand", color = "#C6227F") +   
  # annotate("text", x = diagnosands$mean_estimate - 0.025, y = 100, hjust = 1, label = "Average estimate", color = gray(0.1)) + 
...
[Get Started](getting_started/){.btn-action-primary  .btn-action .btn .btn-success .btn-lg role="button"}
