...
diagnosis <-
  two_arm_trial |>
  redesign(sample_size = c(250, 500, 750, 1000, 1250, 1500)) |>
  diagnose_designs() |>
  tidy() |>
...
ggplot(diagnosands, aes(sample_size, estimate)) + 
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high), fill = "lightblue", color = NA) + 
  geom_point() +
  geom_line() +
  geom_hline(yintercept = 0.8, color = "red", linewidth = 1, lty = "dashed") + 
