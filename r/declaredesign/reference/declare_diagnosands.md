...
- subset:
  ...
  A subset of the simulations data frame within which to calculate
  diagnosands e.g. `subset = p.value < .05`.
...
Diagnosands summarize the simulations generated by
...
Typically, the columns of the resulting simulations data.frame include
the following variables: estimate, std.error, p.value, conf.low,
conf.high, and inquiry. Many diagnosands will be a function of these
variables.
