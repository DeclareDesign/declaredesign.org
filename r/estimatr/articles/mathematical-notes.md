...
$\left( \mathbf{X}^{\top}\mathbf{X} \right)^{- 1}$ explicitly and
behaves much like the default `lm` function in R. However, when
$\mathbf{X}$ is rank deficient, there are certain conditions under which
the QR factorization algorithm we use, from the Eigen C++ library, drops
different coefficients from the output than the default `lm` function.
In general, users should avoid specifying models with rank-deficiencies.
In fact, if users are certain their data are not rank deficient, they
can improve the speed of `lm_robust` by setting `try_cholesky = TRUE`.
This replaces the QR factorization with a Cholesky factorization that is
only guaranteed to work $\mathbf{X}$ is of full rank.
...
$\mathbf{x}_{i}\sqrt{w_{i}}$, and then do the same to the outcome,
$\mathbf{y}_{i}\sqrt{w_{i}}$. This results in our coefficients being
estimated as follows, where $\mathbf{W}$ is a diagonal matrix with the
scaled weights on the diagonal.

Weighted:
$$\widehat{\beta} = \left( \mathbf{X}^{\top}\mathbf{W}\mathbf{X} \right)^{- 1}\mathbf{X}^{\top}\mathbf{W}\mathbf{y}$$

The transformed data are then used in the analysis below, where
$\left( \mathbf{X}^{\top}\mathbf{X} \right)^{- 1}$ is now
$\left( \mathbf{X}^{\top}\mathbf{W}\mathbf{X} \right)^{- 1}$ and
$\mathbf{X}$ is now $\mathbf{X}{sqrt}\lbrack W\rbrack$, where
${sqrt}\lbrack.\rbrack$ is an operator that applies a square root to the
coefficients of some matrix.
...
| `se_type =`        | Variance Estimator ($\widehat{\mathbb{V}}\left\lbrack \widehat{\beta} \right\rbrack$)                                                                                                                      | Degrees of Freedom | Notes                                                     |
|--------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------|-----------------------------------------------------------|
| `"classical"`      | $\frac{\mathbf{e}^{\top}\mathbf{e}}{N - K}\left( \mathbf{X}^{\top}\mathbf{X} \right)^{- 1}$                                                                                                                | N - K              |                                                           |
| `"HC0"`            | $\left( \mathbf{X}^{\top}\mathbf{X} \right)^{- 1}\mathbf{X}^{\top}{diag}\left\lbrack e_{i}^{2} \right\rbrack\mathbf{X}\left( \mathbf{X}^{\top}\mathbf{X} \right)^{- 1}$                                    | N - K              |                                                           |
| `"HC1"`, `"stata"` | $\frac{N}{N - K}\left( \mathbf{X}^{\top}\mathbf{X} \right)^{- 1}\mathbf{X}^{\top}{diag}\left\lbrack e_{i}^{2} \right\rbrack\mathbf{X}\left( \mathbf{X}^{\top}\mathbf{X} \right)^{- 1}$                     | N - K              | Often called the Eicker-Huber-White variance (or similar) |
| `"HC2"` (default)  | $\left( \mathbf{X}^{\top}\mathbf{X} \right)^{- 1}\mathbf{X}^{\top}{diag}\left\lbrack \frac{e_{i}^{2}}{1 - h_{ii}} \right\rbrack\mathbf{X}\left( \mathbf{X}^{\top}\mathbf{X} \right)^{- 1}$                 | N - K              |                                                           |
| `"HC3"`            | $\left( \mathbf{X}^{\top}\mathbf{X} \right)^{- 1}\mathbf{X}\top{diag}\left\lbrack \frac{e_{i}^{2}}{\left( 1 - h_{ii} \right)^{2}} \right\rbrack\mathbf{X}\left( \mathbf{X}^{\top}\mathbf{X} \right)^{- 1}$ | N - K              |                                                           |

- $\mathbf{x}_{i}$ is the $i$th row of $\mathbf{X}$.
- $h_{ii} = \mathbf{x}_{i}\left( \mathbf{X}^{\top}\mathbf{X} \right)^{- 1}\mathbf{x}_{i}^{\top}$
- $e_{i} = y_{i} - \mathbf{x}_{i}\widehat{\beta}$
- ${diag}\lbrack.\rbrack$ is an operator that creates a diagonal matrix
  from a vector
...
| `se_type =`       | Variance Estimator ($\widehat{\mathbb{V}}\left\lbrack \widehat{\beta} \right\rbrack$)                                                                                                                                                                  | Degrees of Freedom                                                                                                                                                      | Notes                                                                                                                                                                                                                                                                                                                                                                                               |
|-------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `"CR0"`           | $\left( \mathbf{X}^{\top}\mathbf{X} \right)^{- 1}\sum_{s = 1}^{S}\left\lbrack \mathbf{X}_{s}^{\top}\mathbf{e}_{s}\mathbf{e}_{s}^{\top}\mathbf{X}_{s} \right\rbrack\left( \mathbf{X}^{\top}\mathbf{X} \right)^{- 1}$                                    | $S - 1$                                                                                                                                                                 |                                                                                                                                                                                                                                                                                                                                                                                                     |
| `"stata"`         | $\frac{N - 1}{N - K}\frac{S}{S - 1}\left( \mathbf{X}^{\top}\mathbf{X} \right)^{- 1}\sum_{s = 1}^{S}\left\lbrack \mathbf{X}_{s}^{\top}\mathbf{e}_{s}\mathbf{e}_{s}^{\top}\mathbf{X}_{s} \right\rbrack\left( \mathbf{X}^{\top}\mathbf{X} \right)^{- 1}$  | $S - 1$                                                                                                                                                                 | The Stata variance estimator is the same as the CR0 estimate but with a special finite-sample correction.                                                                                                                                                                                                                                                                                           |
| `"CR2"` (default) | $\left( \mathbf{X}^{\top}\mathbf{X} \right)^{- 1}\sum_{s = 1}^{S}\left\lbrack \mathbf{X}_{s}^{\top}\mathbf{A}_{s}\mathbf{e}_{s}\mathbf{e}_{s}^{\top}\mathbf{A}_{s}^{\top}\mathbf{X}_{s} \right\rbrack\left( \mathbf{X}^{\top}\mathbf{X} \right)^{- 1}$ | $\frac{\left( \sum_{i = 1}^{S}\mathbf{p}_{i}^{\top}\mathbf{p}_{i} \right)^{2}}{\sum_{i = 1}^{S}\sum_{j = 1}^{S}\left( \mathbf{p}_{i}^{\top}\mathbf{p}_{j} \right)^{2}}$ | These estimates of the variance and degrees of freedom come from Pustejovsky and Tipton ([2018](#ref-pustejovskytipton2016)), which is an extension to certain models with a particular set of dummy variables of the method proposed by Bell and McCaffrey ([2002](#ref-bellmccaffrey2002)). Note that the degrees of freedom can vary for each coefficient. See below for more complete notation. |

- $S$ is the number of clusters
- $\mathbf{X}_{s}$ is the rows of $\mathbf{X}$ that belong to cluster
  $s$
- $I_{n}$ is an identity matrix of size $n \times n$
- $\mathbf{e}_{s}$ is the elements of the residual matrix $\mathbf{e}$
  in cluster $s$, or
  $\mathbf{e}_{s} = \mathbf{y}_{s} - \mathbf{X}_{s}\widehat{\beta}$
- $\mathbf{A}_{s}$ and $\mathbf{p}$ are defined in the notes below
...
when $\mathbf{B}_{\mathbf{s}}$ is full rank. It could be rank-deficient
if there were dummy variables, or fixed effects, that were also your
clusters. In this case, the original Bell and McCaffrey
...
In the CR2 variance calculation, we get $\mathbf{A}_{s}$ as follows:

$$\begin{aligned}
\mathbf{H} & {= \mathbf{X}\left( \mathbf{X}^{\top}\mathbf{X} \right)^{- 1}\mathbf{X}^{\top}} \\
 & \\
 & \\
\mathbf{B}_{s} & {= \left( I_{N} - \mathbf{H} \right)_{s}\left( I_{N} - \mathbf{H} \right)_{s}^{\top}} \\
 & \\
 & \\
\mathbf{A}_{s} & {= \mathbf{B}_{s}^{+ 1/2}}
\end{aligned}$$

where $\mathbf{B}_{s}^{+ 1/2}$ is the symmetric square root of the
Moore–Penrose inverse of $\mathbf{B}_{s}$ and $(I - \mathbf{H})_{s}$ are
the $N_{s}$ columns that correspond to cluster $s$. To get the
corresponding degrees of freedom, note that

$$\mathbf{p}_{s} = \left( I_{N} - \mathbf{H} \right)_{s}^{\top}\mathbf{A}_{s}\mathbf{X}_{s}\left( \mathbf{X}^{\top}\mathbf{X} \right)^{- 1}\mathbf{z}_{k}$$
where $\mathbf{z}_{k}$ is a vector of length $K$, the number of
coefficients, where the $k$th element is 1 and all other elements are 0.
The $k$ signifies the coefficient for which we are computing the degrees
of freedom.

#### Confidence intervals and hypothesis testing

If ${\widehat{\mathbb{V}}}_{k}$ is the $k$th diagonal element of
$\widehat{\mathbb{V}}$, we build confidence intervals using the user
specified $\alpha$ as:

$${CI}^{1 - \alpha} = \left( \widehat{\beta_{k}} + t_{\alpha/2}^{df}\sqrt{{\widehat{\mathbb{V}}}_{k}},\widehat{\beta_{k}} + t_{1 - \alpha/2}^{df}\sqrt{{\widehat{\mathbb{V}}}_{k}} \right)$$

We also provide two-sided p-values using a t-distribution with the
aforementioned significance level $\alpha$ and degrees of freedom $df$.
...
$$y_{i} = \tau z_{i} + {\mathbf{β}}^{\top}\mathbf{x}_{i} + \epsilon_{i}$$

with the following model

$$y_{i} = \tau z_{i} + {\mathbf{β}}^{\top}\mathbf{x}_{i}^{c} + {\mathbf{γ}}^{\top}\mathbf{x}_{i}^{c}z_{i} + \epsilon_{i}$$

where $\mathbf{x}_{i}^{c}$ is a vector of pre-treatment covariates for
unit $i$ that have been centered to have mean zero and $z_{i}$ is an
...
$${\widehat{\beta}}_{2SLS} = \left( \mathbf{X}^{\top}\mathbf{P}_{\mathbf{z}}\mathbf{X} \right)^{- 1}\mathbf{X}^{\top}\mathbf{P}_{\mathbf{z}}\mathbf{y},$$

where $\mathbf{X}$ are the endogenous regressors,
$\mathbf{P}_{\mathbf{Z}} = \mathbf{Z}\left( \mathbf{Z}^{\top}\mathbf{Z} \right)^{- 1}\mathbf{Z}^{\top}$,
and $\mathbf{Z}$ are the instruments. This is equivalent to estimating
the first stage regression,

$$\mathbf{X} = \mathbf{Z}\beta_{FS} + {\mathbf{ζ}},$$
...
\mathbf{y} & {= \widehat{\mathbf{X}}\beta_{2SLS} + {\mathbf{ϵ}}.}
\end{aligned}$$
...
$\mathbf{X}$ with the second stage regressors, $\widehat{\mathbf{X}}$,
and we replace the residuals, $e_{i}$, with
...
$${CI}^{1 - \alpha} = \left( \widehat{\beta_{2SLS,k}} + t_{\alpha/2}^{df}\sqrt{{\widehat{\mathbb{V}}}_{k}},\widehat{\beta_{2SLS,k}} + t_{1 - \alpha/2}^{df}\sqrt{{\widehat{\mathbb{V}}}_{k}} \right)$$
...
Stata uses z-tests when `small` is not specified.
...
$$\widehat{\tau} = \frac{1}{N}\sum\limits_{i = 1}^{N}z_{i}y_{i} - \left( 1 - z_{i} \right)y_{i}$$
where $z_{i}$ is the treatment variable, $y_{i}$ is the outcome, and $N$
is the total number of units.

**Blocked design (including matched-pairs designs)**
$$\widehat{\tau} = \sum\limits_{j = 1}^{J}\frac{N_{j}}{N}\widehat{\tau_{j}}$$
where $J$ is the number of blocks, $N_{j}$ is the size of those blocks,
and $\widehat{\tau_{j}}$ is the estimated difference-in-means in block
$j$.
...
| Design type                      | Variance $\widehat{\mathbb{V}}\left\lbrack \widehat{\tau} \right\rbrack$                                                                      | Degrees of Freedom                                                                                                                                                                                                                                                                      | Notes                                                                                                                                                                                                                                                                                                                                                                                                               |
|----------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| No blocks or clusters (standard) | $\frac{\widehat{\mathbb{V}}\left\lbrack y_{i,0} \right\rbrack}{N_{0}} + \frac{\widehat{\mathbb{V}}\left\lbrack y_{i,1} \right\rbrack}{N_{1}}$ | $\widehat{\mathbb{V}}\left\lbrack \widehat{\tau} \right\rbrack^{2}\left( \frac{\left( \widehat{\mathbb{V}}\left\lbrack y_{i,1} \right\rbrack/N_{1} \right)^{2}}{N_{1} - 1} + \frac{\left( \widehat{\mathbb{V}}\left\lbrack y_{i,0} \right\rbrack/N_{0} \right)^{2}}{N_{0} - 1} \right)$ | Where $\widehat{\mathbb{V}}\left\lbrack y_{i,k} \right\rbrack$ is the Bessel-corrected variance of all units where $z_{i} = k$ and $N_{k}$ is the number of units in condition $k$. This is equivalent to the variance and Welch–Satterthwaite approximation of the degrees of freedom used by R’s `t.test`.                                                                                                        |
| Blocked                          | $\sum_{j = 1}^{J}\left( \frac{N_{j}}{N} \right)^{2}\widehat{\mathbb{V}}\left\lbrack \widehat{\tau_{j}} \right\rbrack$                         | $N - 2*J$                                                                                                                                                                                                                                                                               | Where $\widehat{\mathbb{V}}\left\lbrack \widehat{\tau_{j}} \right\rbrack$ is the variance of the estimated difference-in-means in block $j$. See footnote 17 on page 74 of ([Gerber and Green 2012](#ref-gerbergreen2012)) for a reference. The degrees of freedom are equivalent to a regression with a full set of block specific treatment effects.                                                              |
...
| Blocked and clustered            | $\sum_{j = 1}^{J}\left( \frac{N_{j}}{N} \right)^{2}\widehat{\mathbb{V}}\left\lbrack \widehat{\tau_{j}} \right\rbrack$                         | $S - 2*J$                                                                                                                                                                                                                                                                               | Where $\widehat{\mathbb{V}}\left\lbrack \widehat{\tau_{j}} \right\rbrack$ is the variance of the estimated difference-in-means in block $j$ and S is the number of clusters. See footnote 17 on page 74 of Gerber and Green ([2012](#ref-gerbergreen2012)) for a reference. The degrees of freedom are equivalent to a regression on data collapsed by cluster with a full set of block specific treatment effects. |
| Matched pairs                    | $\frac{1}{J(J - 1)}\sum_{j = 1}^{J}\left( \widehat{\tau_{j}} - \widehat{\tau} \right)^{2}$                                                    | $J - 1$                                                                                                                                                                                                                                                                                 | See equation 3.16 on page 77 of Gerber and Green ([2012](#ref-gerbergreen2012)) for a reference.                                                                                                                                                                                                                                                                                                                    |
| Matched pair cluster randomized  | $\frac{J}{(J - 1)N^{2}}\sum_{j = 1}^{J}\left( N_{j}\widehat{\tau_{j}} - \frac{N\widehat{\tau}}{J} \right)^{2}$                                | $J - 1$                                                                                                                                                                                                                                                                                 | See the variance for the SATE defined in equation 6 on page 36 of ([Imai, King, and Nall 2009](#ref-imaietal2009)) and the suggested degrees of freedom on page 37.                                                                                                                                                                                                                                                 |

#### Confidence intervals and hypothesis testing

We build confidence intervals using the user specified $\alpha$ as:

$${CI}^{1 - \alpha} = \left( \widehat{\tau} + t_{\alpha/2}^{df}\sqrt{\widehat{\mathbb{V}}\left\lbrack \widehat{\tau} \right\rbrack},\widehat{\tau}\rbrack + t_{1 - \alpha/2}^{df}\sqrt{\widehat{\mathbb{V}}\left\lbrack \widehat{\tau} \right\rbrack} \right)$$
...
- $\pi_{zi}$ is the marginal probability of being in condition
  $z \in \{ 0,1\}$ for unit i
- $\pi_{ziwj}$ is the joint probability of unit $i$ being in condition
  $z$ and unit $j$ being in condition $w \in \{ 0,1\}$
...
$$\widehat{\tau} = \frac{1}{N}\sum\limits_{i = 1}^{N}z_{i}\frac{y_{i}}{\pi_{1i}} - \left( 1 - z_{i} \right)\frac{y_{i}}{\pi_{0i}}$$

**Blocked**

$$\widehat{\tau} = \sum\limits_{j = 1}^{J}\frac{N_{j}}{N}\widehat{\tau_{j}}$$
where $J$ is the number of blocks, $N_{j}$ is the size of those blocks,
and $\widehat{\tau_{j}}$ is the Horvitz-Thompson estimate in block $j$.
...
$$\begin{aligned}
{{\widehat{\mathbb{V}}}_{Y}\left\lbrack \widehat{\tau} \right\rbrack = \frac{1}{N^{2}}\sum\limits_{i = 1}^{N}\lbrack} & {z_{i}\left( \frac{y_{i}}{\pi_{1i}} \right)^{2} + \left( 1 - z_{i} \right)\left( \frac{y_{i}}{\pi_{0i}} \right)^{2} + \sum\limits_{j \neq i}(\frac{z_{i}z_{j}}{\pi_{1i1j} + \epsilon_{1i1j}}\left( \pi_{1i1j} - \pi_{1i}\pi_{1j} \right)\frac{y_{i}}{\pi_{1i}}\frac{y_{j}}{\pi_{1j}}} \\
 & \\
 & \\
 & {+ \frac{\left( 1 - z_{i} \right)\left( 1 - z_{j} \right)}{\pi_{0i0j} + \epsilon_{0i0j}}\left( \pi_{0i0j} - \pi_{0i}\pi_{0j} \right)\frac{y_{i}}{\pi_{0i}}\frac{y_{j}}{\pi_{0j}} - 2\frac{z_{i}\left( 1 - z_{j} \right)}{\pi_{1i0j} + \epsilon_{1i0j}}\left( \pi_{1i0j} - \pi_{1i}\pi_{0j} \right)\frac{y_{i}}{\pi_{1i}}\frac{y_{j}}{\pi_{0j}}} \\
 & \\
 & \\
 & {+ \sum\limits_{\forall j:\pi_{1i1j} = 0}\left( z_{i}\frac{y_{i}^{2}}{2\pi_{1i}} + z_{j}\frac{y_{j}^{2}}{\pi_{1j}} \right) + \sum\limits_{\forall j:\pi_{0i0j} = 0}\left( \left( 1 - z_{i} \right)\frac{y_{i}^{2}}{2\pi_{0i}} + \left( 1 - z_{j} \right)\frac{y_{j}^{2}}{\pi_{0j}} \right)\rbrack}
\end{aligned}$$
...
$$\begin{aligned}
{{\widehat{\mathbb{V}}}_{Y}\left\lbrack \widehat{\tau} \right\rbrack = \frac{1}{N^{2}}\sum\limits_{i = 1}^{N}\lbrack} & {z_{i}\left( \frac{y_{i}}{\pi_{1i}} \right)^{2} + \left( 1 - z_{i} \right)\left( \frac{y_{i}}{\pi_{0i}} \right)^{2} + \sum\limits_{j \neq i}(\frac{z_{i}z_{j}}{\pi_{1i1j}}\left( \pi_{1i1j} - \pi_{1i}\pi_{1j} \right)\frac{y_{i}}{\pi_{1i}}\frac{y_{j}}{\pi_{1j}}} \\
 & \\
 & \\
 & {+ \frac{\left( 1 - z_{i} \right)\left( 1 - z_{j} \right)}{\pi_{0i0j}}\left( \pi_{0i0j} - \pi_{0i}\pi_{0j} \right)\frac{y_{i}}{\pi_{0i}}\frac{y_{j}}{\pi_{0j}} - 2\frac{z_{i}\left( 1 - z_{j} \right)}{\pi_{1i0j}}\left( \pi_{1i0j} - \pi_{1i}\pi_{0j} \right)\frac{y_{i}}{\pi_{1i}}\frac{y_{j}}{\pi_{0j}}\rbrack}
\end{aligned}$$
...
$$\begin{aligned}
{{\widehat{\mathbb{V}}}_{Y}\left\lbrack \widehat{\tau} \right\rbrack = \frac{1}{N^{2}}\sum\limits_{i = 1}^{N}\lbrack} & {z_{i}\left( \frac{y_{i}}{\pi_{1i}} \right)^{2} + \left( 1 - z_{i} \right)\left( \frac{y_{i}}{\pi_{0i}} \right)^{2}\rbrack}
\end{aligned}$$
...
$y_{k}$ is the total of the outcomes $y_{i}$ for all $i$ units in
cluster $k$, $\pi_{z}k$ is the marginal probability of cluster $k$ being
in condition $z \in \{ 0,1\}$, and $z_{k}$ and $\pi_{zkwl}$ are defined
analogously. Warning! If one passes `condition_pr_mat` to
...
$$\begin{aligned}
{{\widehat{\mathbb{V}}}_{Y}\left\lbrack \widehat{\tau} \right\rbrack = \frac{1}{N^{2}}\sum\limits_{k = 1}^{M}\lbrack} & {z_{k}\left( \frac{y_{k}}{\pi_{1k}} \right)^{2} + \left( 1 - z_{k} \right)\left( \frac{y_{k}}{\pi_{0k}} \right)^{2} + \sum\limits_{l \neq k}(\frac{z_{k}z_{l}}{\pi_{1k1l} + \epsilon_{1k1l}}\left( \pi_{1k1l} - \pi_{1k}\pi_{1l} \right)\frac{y_{k}}{\pi_{1k}}\frac{y_{l}}{\pi_{1l}}} \\
 & \\
 & \\
 & {+ \frac{\left( 1 - z_{k} \right)\left( 1 - z_{l} \right)}{\pi_{0k0l} + \epsilon_{0k0l}}\left( \pi_{0k0l} - \pi_{0k}\pi_{0l} \right)\frac{y_{k}}{\pi_{0k}}\frac{y_{l}}{\pi_{0l}} - 2\frac{z_{k}\left( 1 - z_{l} \right)}{\pi_{1k0l} + \epsilon_{1k0l}}\left( \pi_{1k0l} - \pi_{1k}\pi_{0l} \right)\frac{y_{k}}{\pi_{1k}}\frac{y_{l}}{\pi_{0l}}} \\
 & \\
 & \\
 & {+ \sum\limits_{\forall l:\pi_{1k1l} = 0}\left( z_{k}\frac{y_{k}^{2}}{2\pi_{1k}} + z_{l}\frac{y_{l}^{2}}{\pi_{1l}} \right) + \sum\limits_{\forall l:\pi_{0k0l} = 0}\left( \left( 1 - z_{k} \right)\frac{y_{k}^{2}}{2\pi_{0k}} + \left( 1 - z_{l} \right)\frac{y_{l}^{2}}{\pi_{0l}} \right)\rbrack}
\end{aligned}$$
...
- $y_{zi}$ is the potential outcome for condition $z$ for unit $i$. This
  is either observed if $z_{i} = z$ or estimated using the constant
  effects assumption if $z_{i} \neq z$, where $z_{i}$ is the condition
  for unit $i$. To be precise
  $y_{1i} = z_{i}y_{i} + \left( 1 - z_{i} \right)\left( y_{i} + \widehat{\tau} \right)$
  and
  $y_{0i} = z_{i}\left( y_{i} - \widehat{\tau} \right) + \left( 1 - z_{i} \right)y_{i}$,
  where $\widehat{\tau}$ is the estimated treatment effect.

$$\begin{aligned}
{{\widehat{\mathbb{V}}}_{C}\left\lbrack \widehat{\tau} \right\rbrack = \frac{1}{N^{2}}\sum\limits_{i = 1}^{N}\lbrack} & {\left( 1 - \pi_{0i} \right)\pi_{0i}\left( \frac{y_{0i}}{\pi_{0i}} \right)^{2} + \left( 1 - \pi_{1i} \right)\pi_{1i}\left( \frac{y_{1i}}{\pi_{1i}} \right)^{2} - 2y_{1i}y_{0i}} \\
 & \\
 & \\
 & {+ \sum\limits_{j \neq i}(\left( \pi_{0i0j} - \pi_{0i}\pi_{0j} \right)\frac{y_{0i}}{\pi_{0i}}\frac{y_{0j}}{\pi_{0j}} + \left( \pi_{1i1j} - \pi_{1i}\pi_{1j} \right)\frac{y_{1i}}{\pi_{1i}}\frac{y_{1j}}{\pi_{1j}}} \\
 & \\
 & \\
 & {- 2\left( \pi_{1i0j} - \pi_{1i}\pi_{0j} \right)\frac{y_{1i}}{\pi_{1i}}\frac{y_{0j}}{\pi_{0j}})\rbrack}
\end{aligned}$$
...
$${CI}^{1 - \alpha} = \left( \widehat{\tau} + z_{\alpha/2}\sqrt{\widehat{\mathbb{V}}\left\lbrack \widehat{\tau} \right\rbrack},\widehat{\tau} + z_{1 - \alpha/2}\sqrt{\widehat{\mathbb{V}}\left\lbrack \widehat{\tau} \right\rbrack} \right)$$
...
Pair Matching in Cluster-Randomized Experiments, with Application to the
