...
write_rds(diagnosis, file = "rfiles/11_model_spillovers_1.rds")
```
...
get_diagnosands(diagnosis) %>% 
  ggplot(aes(x = estimator, y = power, color = estimator)) +
  geom_hline(aes(yintercept = power, color = estimator), alpha = .3, linetype = "dashed") +
...
#  mutate(estimator = factor(inquiry, levels = c("naive", "saturated"))) %>%
  ggplot(aes(x = inquiry, y = power, color = inquiry)) +
  geom_hline(aes(yintercept = power, color = inquiry), alpha = .3, linetype = "dashed") +
